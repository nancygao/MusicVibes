    ...
    while 1:
        ...
        if data==b'\x00':
            ...
            try:
                ...
                print('Key '+key+' not found in dictionary', file=sys.stderr)
        elif data==b'\x01':
            key = conn.recv(1)[0]
            print(key)
            try:
                byte = new_dict[key]
                #print(str.format(byte,"02x"))
                ser_only_one(ser, byte)
            except:
                print('Key '+key+' not found in dictionary', file=sys.stderr)
        elif data==b'\x02':
            try:
                byte_1 = new_dict[conn.recv(1)[0]]
                byte_2 = new_dict[conn.recv(1)[0]]
                ser_init(ser, byte_1, byte_2)
            except:
                print('Key not found in dictionary', file = sys.stderr)
        elif data==b'\x03':
            try:
                byte = new_dict[conn.recv(1)[0]]
            ...
            my_enqueue(ser, byte_queue, new_dict[conn.recv(1)[0]])
            print(byte_queue)
        elif data == b'\x06':
